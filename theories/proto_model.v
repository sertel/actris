...
Definition protoOF_helper (V : Type) (PROPn PROP : ofe) : oFunctor :=
  optionOF (actionO * (V -d> (▶ ∙ -n> PROPn) -n> PROP)).
Definition proto_result (V : Type) (PROPn PROP : ofe) `{!Cofe PROPn, !Cofe PROP} :
...
Definition proto_fold {V} `{!Cofe PROPn, !Cofe PROP} :
    oFunctor_apply (protoOF_helper V PROPn PROP) (proto V PROPn PROP) -n> proto V PROPn PROP :=
  ofe_iso_1 (proto_result V PROPn PROP).
Definition proto_unfold {V} `{!Cofe PROPn, !Cofe PROP} :
    proto V PROPn PROP -n> oFunctor_apply (protoOF_helper V PROPn PROP) (proto V PROPn PROP) :=
  ofe_iso_2 (proto_result V PROPn PROP).
...
Lemma proto_unfold_fold {V} `{!Cofe PROPn, !Cofe PROP}
    (p : oFunctor_apply (protoOF_helper V PROPn PROP) (proto V PROPn PROP)) :
...
Lemma proto_message_equivI `{!BiInternalEq SPROP} {V} `{!Cofe PROPn, !Cofe PROP} a1 a2 pc1 pc2 :
...
Proof.
  ...
  by rewrite discrete_fun_equivI discrete_eq.
Qed.
Lemma proto_message_end_equivI `{!BiInternalEq SPROP} {V} `{!Cofe PROPn, !Cofe PROP} a pc :
...
Qed.
Lemma proto_end_message_equivI `{!BiInternalEq SPROP} {V} `{!Cofe PROPn, !Cofe PROP} a pc :
...
Next Obligation.
  ...
  apply proto_map_ext=> //= y; by rewrite oFunctor_map_compose.
Qed.
