...
From iris.algebra.lib Require Import excl_auth mono_list.
From iris.base_logic.lib Require Import invariants.
From iris.proofmode Require Import tactics.
From iris.heap_lang Require Import primitive_laws.
From actris Require Import step_proto.
...
(** Notation for treating the step update as a view shift. 
    OBS: It seems that this notation may break other Iris notations. *)
Definition step_update_vs `{!irisGS_gen hlc Λ Σ} E1 E2 (P : iProp Σ) : iProp Σ :=
  |={E1,E2}=> |~{E2}~> |={E2,E1}=> P.
Notation "|~{ E1 }[ E2 ]~> P" := (step_update_vs E1 E2 P)
...
Section iProto_sessions.
  Context `{A : Type}.
  Context `{!session_escrowG Σ A, !heapGS Σ}.
  Context (N : namespace).
...
Definition auth_auth_sent (χ : session_escrow_name) (s : side) (n : nat) : iProp Σ :=
  ...
  Proof.
    ...
    iApply step_get_fupd.
    iIntros "!>".
    iDestruct "Hp" as (γc) "(Hctx & Hpl & Hpr)".
  ...
  Proof.
    ...
    - iDestruct (iProto_step_send_r with "Hctx Hp Hpm") as "Hctx".
      iIntros "!>". iApply step_update_fupd. iIntros "!>".
      iDestruct "Hctx" as "[Hctx Hp]".
      ...
      iMod (own_update_2 _ _ _ (●E (S (length Tr)) ⋅ ◯E (S (length Tr)))
             with "Hsr HsF") as "[Hsr HsF]"; [by apply excl_auth_update|].
  ...
  Lemma step_update_recv_l χ E n m pm v :
    ...
    |~{E}[E∖↑N]~> ∃ p, ses_own χ Left n (S m) p ∗ iMsg_car pm v (Next p).
  Proof.
    rewrite !ses_own_eq !ses_idx_eq.
    ...
    iModIntro.
    iDestruct (iProto_step_recv_l with "Hctx Hp") as "Hctx". iIntros "!>".
    iDestruct "Hctx" as (p) "(Hctx & Hp &HP)".
  ...
  Lemma step_update_recv_r χ E n m pm v :
    ...
    |~{E}[E∖↑N]~> ∃ p, ses_own χ Right n (S m) p ∗ iMsg_car pm v (Next p).
  Proof.
    rewrite !ses_own_eq !ses_idx_eq.
    ...
    iModIntro.
    iDestruct (iProto_step_recv_r with "Hctx Hp") as "Hctx". iIntros "!>".
    iDestruct "Hctx" as (p) "(Hctx & Hp &HP)".
    iMod (own_update_2 _ _ _ (●E (S m) ⋅ ◯E (S m))
  ...
  Lemma step_update_recv χ s E n m pm v :
    ...
    |~{E}[E∖↑N]~> ∃ p, ses_own χ s n (S m) p ∗ iMsg_car pm v (Next p).
  Proof.
    iIntros (HE) "Hp Hpm".
    destruct s.
