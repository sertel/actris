...
Section channel.
  ...
  Lemma try_recv_spec_packed_atomic
    ...
    <<< ∃ v, (⌜v = NONEV⌝ ∧ c ↣ iProto_message Recv pc) ∨
              ▷ (∃ x : TT, ⌜v = SOMEV ((pc x).1.1)⌝ ∗
                           c ↣ (pc x).2 ∗ ▷ (pc x).1.2),
    RET v >>>.
  Proof.
    rewrite iProto_mapsto_eq. iIntros (Φ) "HAU".
    iApply fupd_wp. iMod "HAU" as "[Hp [Habort _]]".
    iDestruct "Hp" as (γ s l r lk) "(#Heq & #Hm1 & #Hm2 & #Hm3 & #Hlk & Hown)".
    iMod ("Habort" with "[-]") as "HAU".
    { iExists γ, s, l, r, lk. eauto 10 with iFrame. }
    iModIntro.
    wp_lam; wp_pures.
    iDestruct "Heq" as %->.
    wp_apply (acquire_spec with "Hlk"); iIntros "[Hlkd Hinv]".
    ...
    - destruct vsr as [|vr vsr].
      ...
      iMod ("Hcomm" $!(InjRV v') with "[H]") as "Hcomm".
      {
        iRight. iNext.
        iDestruct "H" as (x ->) "[Hown Hpc]".
  ...
  Proof.
    iIntros (Φ) "Hp HΦ".
    wp_apply (atomic_wp_seq_step $! (try_recv_spec_packed_atomic c pc)
                   with "[$Hp]")=> //.
  Qed.
