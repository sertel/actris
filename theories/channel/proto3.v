...
Program Definition iProto_consistent_pre' {Σ V}
  ...
  list V -d> list V -d> iProto Σ V -n> iProto Σ V -n> iPropO Σ :=
  λ l r, λne p1 p2, iProto_consistent_pre (λ l r p1 p2, rec l r p1 p2) l r p1 p2.
Solve Obligations with solve_proper.

Local Instance iProto_consistent_pre_contractive {Σ V} : Contractive (@iProto_consistent_pre' Σ V).
Proof.
  intros n rec1 rec2 Hrec bl br pl pr. rewrite /iProto_consistent_pre' /iProto_consistent_pre /=.
  by repeat (f_contractive || f_equiv).
Qed.

Definition iProto_consistent {Σ V} (bl br : list V) (pl pr : iProto Σ V) : iProp Σ :=
  fixpoint iProto_consistent_pre' bl br pl pr.

Arguments iProto_consistent {_ _} _ _ _%proto _%proto.
Global Instance: Params (@iProto_consistent) 2 := {}.

Global Instance iProto_consistent_ne {Σ V} (bl br : list V) : NonExpansive2 (@iProto_consistent Σ V bl br).
Proof. solve_proper. Qed.
Global Instance iProto_consistent_proper {Σ V} : Proper ((=) ==> (=) ==> (≡) ==> (≡) ==> (⊣⊢)) (@iProto_consistent Σ V).
Proof. solve_proper. Qed.

Lemma iProto_consistent_unfold {Σ V} (bl br : list V) (pl pr : iProto Σ V) :
  iProto_consistent bl br pl pr ≡ (iProto_consistent_pre iProto_consistent) bl br pl pr.
Proof.
...
Fixpoint iProto_app_sends {Σ V} (vs : list V) (p : iProto Σ V) : iProto Σ V :=
  match vs with
  | [] => p
  | v :: vs => <!> MSG v; iProto_app_recvs vs p
end.
...
Definition iProto_ctx `{protoG Σ V}
  ...
  ∃ pl pr,
    ...
    ▷ iProto_consistent vsl vsr pl pr.
...
Section proto.
  ...
  Fixpoint iProto_buffer_pred (vs : list V) p :=
    ...
    | []      => True%I
    | v :: vs =>
        (∃m, ⌜ iProto_app_sends (v :: vs) p ≡ (<!> m) ⌝ ∗
               iMsg_car m v (Next (iProto_app_sends vs p)) ∗
  ...
  Proof.
    ...
    rewrite iProto_consistent_unfold /iProto_consistent_pre.
    iDestruct "Hprot" as "(Hend1&Hend2&Hprog&Hprot)".
    iSplit; [done|].
    iSplit; [done|].
    iSplit.
    { iIntros "H1 H2".
      iDestruct ("Hprog" with "H2 H1") as "[Hprog|[Hprog|[H1 H2]]]";
        [by iRight;iLeft|by iLeft|by iRight;iRight;eauto]. }
    iSplit.
  ...
  Proof.
    ...
    iSplit; [by rewrite iProto_message_end_equivI|].
    iSplit.
    { rewrite iProto_consistent_unfold /iProto_consistent_pre.
      iDestruct "Hprot" as "(_&$&_)". }
    iSplit; [by eauto|].
    iSplit.
    + iIntros (a m) "Hm".
    ...
    + rewrite iProto_consistent_unfold /iProto_consistent_pre.
      iDestruct "Hprot" as "(_&_&_&_&Hprot)".
      simpl.
      iIntros (a m) "Hm".
      iDestruct ("Hprot" with "Hm") as "Hprot".
      destruct a.
  ...
  Proof.
    ...
    iSplit; [done|].
    iSplit.
    { iIntros "H". iDestruct (iProto_message_end_equivI with "H") as "[]". }
    iSplit; [eauto|].
    iSplit.
    - iDestruct "Hprot" as "[Hprot _]".
      iIntros (a m') "Hm'".
  ...
  Proof.
    ...
    iSplit.
    { iIntros "H1 H2".
      destruct (iProto_case p) as [Hp|[a [m Hp]]].
      { setoid_rewrite Hp. setoid_rewrite iProto_dual_end.
        iRight. iRight. done. }
      setoid_rewrite Hp.
      destruct a.
      { iDestruct "H1" as "[H1|H1]".
        { iDestruct (iProto_message_end_equivI with "H1") as "[]". }
        iDestruct "H1" as (m') "H1".
        by iDestruct (iProto_message_equivI with "H1") as (Heq) "H1". }
      setoid_rewrite iProto_dual_message=> /=.
      iDestruct "H2" as "[H2|H2]".
      { iDestruct (iProto_message_end_equivI with "H2") as "[]". }
      iDestruct "H2" as (m') "H2".
      by iDestruct (iProto_message_equivI with "H2") as (Heq) "H2". }      
    repeat iSplit.
    { iIntros (a m) "#Heq".
      destruct a; simpl.
      { iIntros (v p') "H".
  ...
  Lemma iProto_consistent_send vl ml vsl vsr pr pl' :
    iProto_consistent vsl vsr (<!> ml) pr -∗
    iMsg_car ml vl (Next pl') -∗
    ▷ iProto_consistent (vsl ++ [vl]) vsr pl' pr.
  Proof.
    iIntros "Hprot Hm".
    rewrite iProto_consistent_unfold /iProto_consistent_pre.
    iDestruct "Hprot" as "(_&_&_&Hprot&_)".
  ...
  Proof.
    ...
    rewrite iProto_consistent_unfold /iProto_consistent_pre.
    iDestruct "Hprot" as "(_&_&_&_&Hprot)".
    by iSpecialize ("Hprot" with "[//] [//]")=> /=.
  Qed.

  Lemma iProto_consistent_le_l vsl vsr pl pl' pr :
    iProto_consistent vsl vsr pl pr -∗ pl ⊑ pl' -∗ iProto_consistent vsl vsr pl' pr.
  Proof. iIntros "Hprot Hle". by iApply "Hle". Qed.
  Lemma iProto_consistent_le_r vsl vsr pl pr pr' :
    iProto_consistent vsl vsr pl pr -∗ pr ⊑ pr' -∗ iProto_consistent vsl vsr pl pr'.
  Proof.
  ...
  Proof.
    ...
    rewrite iProto_consistent_unfold. unfold iProto_consistent_pre. auto 10 with iFrame.
  Qed.
  ...
  Proof.
    ...
    iDestruct "Hprot" as "(H1&H2&Hprog&Hprot)".
    iSplit.
    { iIntros "H". iDestruct (iProto_message_end_equivI with "H") as "[]". }
    iSplit; [done|].
    iSplit.
    { iIntros "H".
      ...
      iDestruct "H" as (m') "H".
      by iDestruct (iProto_message_equivI with "H") as (Heq) "H". }
  ...
  Proof.
    ...
    iSplit.
    { iIntros "H1' H2'".
      iDestruct ("Hprog" with "[] [H2']") as "[Hprog|[Hprog|Hprog]]".
      { by eauto. }
      { iDestruct "H2'" as "[H2'|H2']".
        - eauto. 
        - iDestruct "H2'" as (m) "H2'". eauto. }
      - eauto.
      - iDestruct "Hprog" as (m) "[Heq Hprog]".
        iRight. iLeft. iExists m2. eauto.
      - iDestruct "Hprog" as "[Hprog _]".
        iDestruct (iProto_message_end_equivI with "Hprog") as "[]".
    }
    iSplit; last first.
    {
      iDestruct "Hprot" as "[_ Hprot]".
    ...
    iDestruct "Hprot" as "[Hprot _]".
    iDestruct ("Hprot" with "[//] [//]") as (p') "[Hm Hprot]".
    iDestruct ("Hle" with "Hm") as (p'') "[Hle H]".
    iExists p''.
  ...
  Lemma iProto_le_refl p :
    ⊢ p ⊑ p.
  Proof. by iIntros (vl vr p') "Hp". Qed.
  ...
  Proof.
    ...
    iEval (rewrite !iProto_consistent_unfold /iProto_consistent_pre /=).
    iSplit.
    { iIntros "Heq". by rewrite iProto_message_end_equivI. }
    iSplit.
    { rewrite iProto_consistent_unfold /iProto_consistent_pre /=.
      by iDestruct "H" as "(_&H&_)". }
    iSplit.
    {
      iIntros "[H1|H1]".
      - iDestruct (iProto_message_end_equivI with "H1") as "[]".
      - iDestruct "H1" as (m) "H1".
        rewrite iProto_message_equivI. by iDestruct "H1" as (Heq) "H1".
    }
    iSplit.
    { iIntros (a m) "Heq".
      iDestruct (iProto_message_equivI with "Heq") as "[<- #Heq]".
      iIntros (v p1') "Hp'".
      iRewrite -("Heq" $! v (Next p1')) in "Hp'". iClear "Heq".
      iEval (rewrite {1}iMsg_base_eq) in "Hp'". simpl.
      iDestruct "Hp'" as "(->&Heq&HP2)".
      ...
      { admit. }
      rewrite iProto_consistent_unfold /iProto_consistent_pre /=.
      iDestruct "H" as "(Hl&Hr&Hprog&[H _])".
      iDestruct ("H" with "[//]") as "H".
      iDestruct ("H" with "[//]") as (p2') "[Heq H]".
      iEval (rewrite {1}iMsg_base_eq) in "Heq". simpl.
      iDestruct "Heq" as (->) "[Heq HP1]".
      iIntros "!>". iRewrite -"Heq" in "H".
      iLöb as "IH2" forall (vl vr p p3).
      iEval (rewrite !iProto_consistent_unfold /iProto_consistent_pre /=).
      iSplit.
      { iIntros "Heq".
        by iDestruct (iProto_message_end_equivI with "Heq") as "Heq". }
      iSplit.
      { iIntros "Heq". iDestruct ("Hr" with "Heq") as %->.
        rewrite iProto_consistent_unfold /iProto_consistent_pre /=.
        iDestruct "H" as "(_&_&_&H&_)".
        iDestruct ("H" with "[//] [HP2]") as "H".
        { iEval (rewrite iMsg_base_eq /=). iFrame. eauto. }
        rewrite iProto_consistent_unfold /iProto_consistent_pre /=.
        iDestruct "H" as "(_&Hr&_)".
        iAssert (▷ ⌜[v] = []⌝)%I with "[Heq Hr]" as "H'".
        { iNext. by iApply "Hr". }
        (* Need to strip later. *)
        admit. }
      (* Prove progress property here *)
      iSplit; [admit|].
      iSplit.
      - admit.
      - admit.
  ...
  Proof.
    ...
    iDestruct (iProto_consistent_le_l with "Hconsistent Hle") as "Hconsistent".
    iDestruct (iProto_consistent_send with "Hconsistent [Hm //]")
      as "Hconsistent".
  ...
  Proof.
    ...
    iAssert (▷ (pr ⊑ <!> m))%I
      with "[Hle]" as "{Hp} Hle"; first (iNext; by iRewrite "Hp").
    iDestruct (iProto_consistent_flip with "Hconsistent") as "Hconsistent".
    iDestruct (iProto_consistent_le_l with "Hconsistent Hle") as "Hconsistent".
    iDestruct (iProto_consistent_send with "Hconsistent [Hm //]") as "Hconsistent".
    iMod (iProto_own_auth_update _ _ _ _ p with "H●r H◯") as "[H●r H◯]".
    iIntros "!>". iSplitR "H◯".
    - iIntros "!>". iExists pl, p. iFrame. by iApply iProto_consistent_flip.
    - iExists p. iFrame. iApply iProto_le_refl.
  Qed.
  ...
  Proof.
    ...
    iDestruct (iProto_consistent_flip with "Hconsistent") as "Hconsistent".
    iDestruct (iProto_consistent_recv with "Hconsistent") as (q) "[Hm Hinterp]".
    iMod (iProto_own_auth_update _ _ _ _ q with "H●l H◯") as "[H●l H◯]".
    iIntros "!> !> /=". iExists q. iFrame "Hm". iSplitR "H◯".
    - iExists q, pr. iFrame. by iApply iProto_consistent_flip.
  ...
  Proof.
    ...
    iDestruct 1 as (p) "[Hle H◯]".
    iDestruct (iProto_own_auth_agree with "H●r H◯") as "#Hp".
    iDestruct (iProto_consistent_le_r with "Hconsistent [Hle]") as "Hconsistent".
    { iIntros "!>". by iRewrite "Hp". }
    iDestruct (iProto_consistent_recv with "Hconsistent") as (q) "[Hm Hconsistent]".
